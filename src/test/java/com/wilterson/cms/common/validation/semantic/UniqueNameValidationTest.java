...
class UniqueNameValidationTest {

    private UniqueNameValidation uniqueNameValidation;

    @BeforeEach
    void setup() {
        uniqueNameValidation = new UniqueNameValidation(new CacheManager(Set.of(new CachedEntity("NAME", "GUID"))));
    }
    ...
    void whenDuplicateName_thenSemanticValidationShouldCatchIssue() {
        ...
        // when
        uniqueNameValidation.validate(merchant, issues);

        // then
        assertThat(issues).hasSize(1);
        IssueAssert.assertThat(new ArrayList<>(issues).getFirst()).hasDescription("Merchant name must be unique.");
    }
}